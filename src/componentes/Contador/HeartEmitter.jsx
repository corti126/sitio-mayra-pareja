...
export const HeartEmitter = ({ triggerCount }) => { // Cambiamos el nombre de la prop a triggerCount
  const [hearts, setHearts] = useState([]);
  // Usamos useRef para generar IDs únicos para cada ráfaga de corazones
  const nextBatchId = useRef(0);

  useEffect(() => {
    // Solo si 'triggerCount' cambia y es mayor que 0 (para evitar que se active al inicio)
    if (triggerCount > 0) { 
  ...
  }, [triggerCount]); // Este efecto se ejecuta cada vez que 'triggerCount' cambia
